...
def calculate_manual_parameters(data, dist_name):
    try:
        if dist_name == 'lognorm':
            ...
            beta = np.min(data)
            if beta < 1e-10:
                raise ValueError("Nilai minimum data (beta) terlalu kecil untuk distribusi Pareto.")
            ...
            alpha = len(data) / np.sum(log_term)  # MLE untuk Pareto
            if np.isnan(alpha) or alpha <= 1:
                raise ValueError("Parameter alpha untuk Pareto tidak valid atau <= 1.")
            return (alpha, 0, beta), {'alpha': alpha, 'beta': beta}
...
def calculate_metrics(data, dist_name, params, seed=42):
    ...
    # Validasi data simulasi untuk Pareto
    if dist_name == 'pareto' and np.any(fitted_data < params[-1]):
        raise ValueError("Data simulasi untuk Pareto mengandung nilai lebih kecil dari beta.")
    ...
    try:
        if dist_name == 'lognorm':
            ...
            if dist_name == 'pareto' and np.any(data < params[-1]):
                raise ValueError("Data mengandung nilai lebih kecil dari beta, tidak valid untuk Pareto.")
...
def fit_distributions(data, distributions, _timeout=60):
    ...
    try:
        ...
        st.error(f"Gagal melakukan fitting distribusi: {str(e)}")
        return None
    ...
    if dist_name == 'lognorm':
        ...
        if dist_name == 'pareto':
            ...
            if alpha <= 0 or beta <= 0:
                raise ValueError("Parameter alpha atau beta untuk Pareto tidak valid.")
...
def allocate_claims(simulated_data, ur, layers):
    ...
    for claim in simulated_data:
        ...
        # Alokasi ke UR
        claim_allocation['UR'] = min(remaining_claims, ur)
    ...
    try:
        ...
        if numeric_columns:
            ...
            if len(data) > 0:
                ...
                if f is None or not f.fitted_param:
